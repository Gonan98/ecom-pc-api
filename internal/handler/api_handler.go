	...
	return func(w http.ResponseWriter, r *http.Request) {
		if err := h(w, r); err != nil {
			if apiError, ok := err.(model.APIError); ok {
				...
				writeError(w, model.APIError{Code: http.StatusInternalServerError, Message: "Internal server error"})
			}
...
func writeError(w http.ResponseWriter, apiErr model.APIError) error {
